...
public class ThemaController {
    ...
    public String getwalking() throws IOException {
        ...
        urlBuilder.append("&" + URLEncoder.encode("MobileOS","UTF-8") + "=" + URLEncoder.encode("ETC", "UTF-8")); /*IOS (아이폰), AND (안드로이드), WIN (윈도우폰), ETC*/
        urlBuilder.append("&" + URLEncoder.encode("MobileApp","UTF-8") + "=" + URLEncoder.encode("AppTest", "UTF-8")); /*서비스명=어플명*/
        urlBuilder.append("&" + URLEncoder.encode("crsKorNm","UTF-8") + "=" + URLEncoder.encode("밀양강", "UTF-8")); /*검색하고자 하는 코스명 (인코딩 필요)*/
        urlBuilder.append("&" + URLEncoder.encode("routeIdx","UTF-8") + "=" + URLEncoder.encode("T_ROUTE_MNG0000000006", "UTF-8")); /*코스에 해당하는 길번호*/
        urlBuilder.append("&" + URLEncoder.encode("crsLevel","UTF-8") + "=" + URLEncoder.encode("2", "UTF-8")); /*코스 난이도(1:하/2:중/3:상)*/
        urlBuilder.append("&" + URLEncoder.encode("brdDiv","UTF-8") + "=" + URLEncoder.encode("DNBW", "UTF-8")); /*걷기/자전거 구분(DNWW : 걷기길, DNBW : 자전거길)*/
    ...
    public String getfestival() throws IOException {
        ...
        StringBuilder urlBuilder = new StringBuilder("http://openapi.culture.go.kr/openapi/service/ArtKnwldgFstvlService/getArtKnwldgFstvl"); /*URL*/
        urlBuilder.append("?" + URLEncoder.encode("ServiceKey","UTF-8") + "=ZyXbs3ggoxjvBgNnPZIbqucXxkimzEhCbTUf2TCX6PoRp%2BTOPYloYNtCYjVBaZzDEm8tpcfCZJW6RVoIRejKpg%3D%3D"); /*Service Key*/
        urlBuilder.append("&" + URLEncoder.encode("ServiceKey","UTF-8") + "=" + URLEncoder.encode("-", "UTF-8")); /*공공데이터포털에서 받은 인증키*/
        urlBuilder.append("&" + URLEncoder.encode("ctgry2","UTF-8") + "=" + URLEncoder.encode("J011", "UTF-8")); /*카테고리2 코드*/
        urlBuilder.append("&" + URLEncoder.encode("ctgry3","UTF-8") + "=" + URLEncoder.encode("J333", "UTF-8")); /*카테고리3 코드*/
        urlBuilder.append("&" + URLEncoder.encode("subject","UTF-8") + "=" + URLEncoder.encode("혜화동1번지", "UTF-8")); /*컨텐츠 제목*/
